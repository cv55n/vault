...
def validate_inputs(prompt, item):
    ...
    for x in required_inputs:
        if x not in inputs:
            return (False, "falta a entrada necessária. {}, {}".format(class_type, x))
        ...
        if isinstance(val, list):
            if len(val) != 2:
                return (False, "input incorreto. {}, {}".format(class_type, x))
            ...
            if r[val[1]] != type_input:
                return (False, "incompatibilidade de tipo de retorno. {}, {}".format(class_type, x))
            ...
            if len(info) > 1:
                if "min" in info[1] and val < info[1]["min"]:
                    return (False, "valor menor que o mínimo. {}, {}".format(class_type, x))
                
                if "max" in info[1] and val > info[1]["max"]:
                    return (False, "valor maior que o máximo. {}, {}".format(class_type, x))

            if isinstance(type_input, list):
                if val not in type_input:
                    return (False, "o valor não está na lista. {}, {}".format(class_type, x))
...
def validate_prompt(prompt):
    ...
    if len(outputs) == 0:
        return (False, "o prompt não tem outputs")
    ...
    for o in outputs:
        ...
        try:
            ...
            valid = False
            reason = "erro de análise"

        if valid == True:
            good_outputs.add(x)
        else:
            print("falha ao validar o prompt para saída {} {}".format(o, reason))
            print("o output será ignorado")

    if len(good_outputs) == 0:
        return (False, "o prompt não possui saídas conectadas corretamente")
...
def run(prompt_queue, address='', port=8188):
    ...
    print("para ver a gui, acesse: http://{}:{}".format(addr, server_address[1]))
