...
GUI::GUI(QWidget *parent, QStringList torrentCmdLine) : QMainWindow(parent) {
    ...
    // s = new session(fingerprint("AZ", 2, 5, 0, 0)); // impressão digital azureus
    ...
    // definição da proporção padrão
    lbl_ratio_icon->setPixmap(QPixmap(QString::fromUtf8(":/Icons/stare.png")));

    // correção do layout da barra de ferramentas
    toolBar->layout()->setSpacing(7);

    // definição modelo de lista de downloads
    ...
    // carrega a largura das últimas colunas para a lista de download
    ...
    // define o nível de severidade da sessão libtorrent
    s->set_severity_level(alert::info);

    // isola algumas exceções
    fs::path::default_name_check(fs::no_check);

    // dht (sem rastreador)
    DHTEnabled = false;

    // configura a sessão bt de acordo com as opções
    configureSession();
    ...
    // retoma os downloads de torrent inacabados
    resumeUnfinished();

    // adiciona o torrent fornecido na linha de comando
    ProcessParams(torrentCmdLine);

    // torna o cabeçalho da lista de downloads clicável para classificação
    downloadList->header()->setClickable(true);
    downloadList->header()->setSortIndicatorShown(true);

    // conectando ações nos slots
    ...
    // inicia a atualização da lista de downloads
    ...
    // adiciona o ícone da bandeja e o personaliza
    ...
    // define o modelo de lista de resultados da pesquisa
    ...
    // torna o cabeçalho da lista de pesquisa clicável para classificação
    resultsBrowser->header()->setClickable(true);
    resultsBrowser->header()->setSortIndicatorShown(true);

    // carrega a largura das últimas colunas para a lista de resultados da pesquisa
    if(!loadColWidthSearchList()) {
        resultsBrowser->header()->resizeSection(0, 275);
    }

    // inicialização booleana
    search_stopped = false;

    // conecta sinais aos slots (parte de pesquisa)
    ...
    // define os nomes de mecanismos de pesquisa
    ...
    // verifica os últimos mecanismos de busca verificados
    ...
    // atualiza o plugin de pesquisa nova.py caso necessário
    updateNova();

    // osd
    OSDWindow = new OSD(this);

    // extensões de visualização suportadas
    // xxx: deve estar incompleto
    ...
    // aceita o drag n drops
    setAcceptDrops(true);

    // configura as informações da barra de informação
    setInfoBar(tr("qbit ")+VERSION+tr(" iniciou."));

    qDebug("gui construída");
}
...
// atualiza a informação da barra de informação
void GUI::setInfoBar(const QString& info, const QString& color) {
    static unsigned short nbLines = 0;

    ++nbLines;

    // verifica o tamanho do log, limpe-o se for muito grande
    if (nbLines > 200) {
        infoBar->clear();
...
// return the row of in data model corresponding to the given filename
...
// Set locale in options, this is required because main() can set the locale and it can't access options object directly.
...
// é executado sempre que as opções são salvas
...
void GUI::processDownloadedFile(QString url, QString file_path, int return_code, QString errorBuffer){
    if(return_code){
        // o download falhou
        setInfoBar(tr("não foi possível baixar", "não foi possível baixar <file>")+" "+url+", "+tr("reason:", "Reason why the download failed")+" "+errorBuffer, "red");
    ...
    // adiciona o arquivo à lista de download de torrent
...
// pega uma string de url para um arquivo torrent, baixe o arquivo torrent para um
// local tmp e o adiciona à lista de downloads
void GUI::downloadFromUrl(const QString& url){
    setInfoBar(tr("baixando", "exemplo: baixando www.example.com/test.torrent")+" '"+url+"', "+tr("por favor aguarde..."), "black");
    
    // inicia o tópico do downloader
    downloader->downloadUrl(url);
    // downloader->start();
}

// exibe uma caixa de diálogo de entrada para solicitar ao usuário uma url
